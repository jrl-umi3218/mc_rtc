...
class TestPythonController(mc_control.MCPythonController):
    ...
    def run_callback(self):
        self.hj1_q = self.robot().mbc.q[self.hj1_index][0]
        self.v3d_data = eigen.Vector3d.Random()
        self.d_data += 1.0
        self.dv_data = [self.d_data] * 3
        self.theta += 0.005
        ...
        if abs(self.d_data - 2.0) < 1e-6:
            self.removeAnchorFrameCallback(
    ...
    def anchorFrameCallback(self, r):
        return sva.interpolate(
            r.surfacePose("LeftFoot"), r.surfacePose("RightFoot"), 0.5
        )
    ...
    def reset_callback(self, reset_data):
        ...
        self.addAnchorFrameCallback(
            ...
            lambda r: sva.interpolate(
                r.surfacePose("LeftFoot"), r.surfacePose("RightFoot"), 0.5
            ),
        )
        self.positionTask.reset()
        self.positionTask.position(
            self.positionTask.position() + eigen.Vector3d(0.1, 0, 0)
        )
        ...
        self.logger().addLogEntry(
            "PYTHONFV",
            lambda: sva.ForceVecd(eigen.Vector6d(0, 0, 0, 0, 0, 100))
            + sva.ForceVecd(eigen.Vector6d(0, 0, 0, 0, 0, 100)),
        )
        ...
        self.gui().addElement(
            ["Python"],
            mc_rtc.gui.Button("button", lambda: sys.stdout.write("Button clicked\n")),
        )
        self.check_ = True
        self.gui().addElement(
            ["Python"], mc_rtc.gui.Checkbox("checkbox", lambda: self.check_, self.check)
        )
        self.mystring_ = "test"
        self.myint_ = 0
        self.mynumber_ = 0.0
        self.gui().addElement(
            ["Python"],
            mc_rtc.gui.StringInput("string input", self.mystring, self.mystring),
        ...
        self.gui().addElement(
            ...
            mc_rtc.gui.NumberSlider(
                "number slider", self.myslider, self.myslider, -100.0, 100.0
            ),
        )
        self.myarray_ = [0.0, 1.0, 2.0, 3.0]
        self.gui().addElement(
            ["Python"], mc_rtc.gui.ArrayInput("array input", self.myarray, self.myarray)
        )
        self.gui().addElement(
            ["Python"],
            mc_rtc.gui.ArrayInput(
                "array input with labels",
                self.myarray,
                self.myarray,
                ["w", "x", "y", "z"],
            ),
        ...
        self.gui().addElement(
            ...
            mc_rtc.gui.ComboInput(
                "combo input", ["a", "b", "c", "d"], self.mycombo, self.mycombo
            ),
        )
        self.mydatacombo_ = ""
        self.gui().addElement(
            ...
            mc_rtc.gui.DataComboInput(
                "data combo", ["robots"], self.mydatacombo, self.mydatacombo
            ),
        )
        self.mypoint3dro_ = [0.0, 1.0, 0.0]
        self.gui().addElement(
            ["Python"], mc_rtc.gui.Point3D("Point3D ro", self.mypoint3dro)
        )
        self.mypoint3d_ = [0.0, 0.0, 1.0]
        self.gui().addElement(
            ["Python"], mc_rtc.gui.Point3D("Point3D", self.mypoint3d, self.mypoint3d)
        )
        self.gui().addElement(
            ["Python", "Form"],
            mc_rtc.gui.Form(
                "Submit",
                lambda c: sys.stdout.write(c.dump(True) + "\n"),
        ...
        self.gui().addElement(
            ["Python", "Experiment"],
            mc_rtc.gui.Button("Add self-destruct button", self.addSelfDestructButton),
        )
    ...
    def addSelfDestructButton(self):
        ...
        self.gui().addElement(
            ["Python", "Experiment"],
            mc_rtc.gui.Button(
                bname, lambda: self.gui().removeElement(["Python", "Experiment"], bname)
            ),
        )
