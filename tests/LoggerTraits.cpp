...
{\
  constexpr bool b = mc_control::log::is_serializable<T>::value;\
  static_assert(b, "Type "#T" is not serializable");\
  constexpr bool b_value_ret = mc_control::log::callback_is_serializable<std::function<T()>>::value;\
  static_assert(b_value_ret, "A function returning "#T" by value could not be serialized");\
  constexpr bool b_ref_ret = mc_control::log::callback_is_serializable<std::function<T&()>>::value;\
  static_assert(b_ref_ret, "A function returning "#T" by reference could not be serialized");\
  constexpr bool b_cref_ret = mc_control::log::callback_is_serializable<std::function<const T&()>>::value;\
  static_assert(b_cref_ret, "A function returning "#T" by const reference could not be serialized");\
  constexpr bool b_vector_is_serializable = mc_control::log::is_serializable<std::vector<T>>::value;\
  constexpr bool b_vector_cref = !b_vector_is_serializable && mc_control::log::callback_is_crv_of_serializable<std::function<const std::vector<T>&()>>::value;\
...
void test_traits()
{
  ...
  constexpr bool b_vector_is_serializable = mc_control::log::is_serializable<std::vector<T>>::value;
  constexpr bool b_vector_cref = (b && b_vector_is_serializable) || mc_control::log::callback_is_crv_of_serializable<std::function<const std::vector<T>&()>>::value;
