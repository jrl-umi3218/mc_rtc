...
#define test_is_serializable(T)                                                                            \
  {                                                                                                        \
    ...
    constexpr bool b_vector_cref =                                                                         \
        !b_vector_is_serializable                                                                          \
        && mc_rtc::log::callback_is_crv_of_serializable<std::function<const std::vector<T> &()>>::value;   \
    static_assert(b_vector_cref,                                                                           \
                  "A function returning a const reference to a vector of " #T " could not be serialized"); \
  }
