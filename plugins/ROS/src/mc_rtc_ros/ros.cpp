...
: nh(nh), rosRate(std::make_unique<rclcpp::Rate>(rate)),
  paramsTopic(
  ...
  odom_pub(this->nh.create_publisher<Odometry>(prefix + "odom", 1)), tf_caster(nh), prefix(prefix), use_real(false),
...
void RobotPublisherImpl::update(double, const mc_rbdyn::Robot & robot)
{
  ...
  for(auto & tf : data.tfs) { tf.header.stamp = data.js.header.stamp; }
...
{
  ROSBridgeImpl() : ros_is_init(ros_init("mc_rtc")), nh(ros_is_init ? rclcpp::Node::make_shared("mc_rtc") : 0) {}
  bool ros_is_init;
