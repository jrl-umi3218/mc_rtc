...
- TVM backend: uses details::TVMCollisionConstraint
      ...
      .def(nb::init<const mc_rbdyn::Robots &, unsigned int, unsigned int, double>(), "robots"_a, "r1Index"_a,
           "r2Index"_a, "timeStep"_a,
           R"(
...
timeStep : float
      ...
      .def_prop_rw(
          "automaticMonitor", [](CollisionsConstraint & cc) { return cc.automaticMonitor(); },
...
If False, the user must manage monitoring manually.)")
      ...
      .def_rw("cols", &CollisionsConstraint::cols, R"(Current set of collisions (list of mc_rbdyn.Collision).)");
}
