...
- TVM backend: no-op
      ...
      // Constructor
      .def(nb::init<double, ContactConstraint::ContactType>(), "timeStep"_a,
...
contactType : ContactConstraint.ContactType, optional
    Type of geometric constraint (Acceleration, Velocity, Position).
      ...
      .def("addToSolver", &ContactConstraint::addToSolverImpl, "solver"_a,
           R"(Add the constraint to the given QPSolver.)")
      .def("removeFromSolver", &ContactConstraint::removeFromSolverImpl, "solver"_a,
           R"(Remove the constraint from the given QPSolver.)")

      .def("contactConstr", &ContactConstraint::contactConstr,
           R"(
