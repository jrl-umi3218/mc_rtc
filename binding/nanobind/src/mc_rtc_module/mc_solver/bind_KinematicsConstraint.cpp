...
• mimic-joint coupling
      ...
      .def(nb::init<const mc_rbdyn::Robot &, const std::array<double, 3> &, double>(), "robot"_a, "damper"_a,
...
velocityPercent : float
    Scaling applied to velocity limits
      ...
      .def("addToSolver", &TVMKinematicsConstraint::addToSolver, "solver"_a,
           "Add the constraint to the given TVMQPSolver")
      .def("removeFromSolver", &TVMKinematicsConstraint::removeFromSolver, "solver"_a,
           "Remove the constraint from the given TVMQPSolver");

  // ------------------------------------------------------------------
  // KinematicsConstraint
  // ------------------------------------------------------------------
  nb::class_<KinematicsConstraint>(m, "KinematicsConstraint",
                                   R"(
...
Uses default damper = {0.1, 0.01, 0.5}, velocityPercent = 0.5.
      ...
      .def(nb::init<const mc_rbdyn::Robots &, unsigned int, double, const std::array<double, 3> &, double>(),
           "robots"_a, "robotIndex"_a, "timeStep"_a, "damper"_a, "velocityPercent"_a,
           R"(
