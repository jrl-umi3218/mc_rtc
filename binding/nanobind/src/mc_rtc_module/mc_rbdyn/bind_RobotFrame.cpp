...
the same interface never has any effect.)";
  ...
  rf.def("robot_const", [](RobotFrame & self) -> const Robot & { return self.robot(); }, nb::rv_policy::reference, "The robot to which this frame is attached (const)")
    .def("robot", [](RobotFrame & self) -> Robot & { return self.robot(); }, nb::rv_policy::reference, "The robot to which this frame is attached (const)");
  ...
  rf.def("bodyMbcIndex", &RobotFrame::bodyMbcIndex, "Returns this frame body index in robot's mbc")
      ...
      .def_prop_rw("X_p_f",
              [](RobotFrame & self) -> const sva::PTransformd & { return self.X_p_f(); },
              [](RobotFrame & self, sva::PTransformd pt) { self.X_p_f(pt); },
              "position"_a,
              "Transformation from the parent's frame/body to the frame")
      .def("hasForceSensor", &RobotFrame::hasForceSensor, "True if the frame has a foce sensor (direct or indirect) attached")
