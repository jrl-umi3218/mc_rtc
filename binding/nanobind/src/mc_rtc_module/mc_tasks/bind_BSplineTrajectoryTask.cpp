...
void bind_BSplineTrajectoryTask(nb::module_ & m)
{
  ...
  // ------------------------------------------------------------------
  nb::class_<BSplineTrajectoryTask>(m, "BSplineTrajectoryTask",
                                    R"(
...
TrajectoryTaskGeneric.
      ...
      .def(nb::init<const mc_rbdyn::RobotFrame &, double, double, double, const sva::PTransformd &, const waypoints_t &,
                    const std::vector<std::pair<double, Eigen::Matrix3d>> &>(),
           "frame"_a, "duration"_a, "stiffness"_a, "weight"_a, "target"_a, "posWp"_a = waypoints_t{},
...
oriWp : list of (float, Matrix3d) pairs, optional
    Waypoints in orientation specified as pairs of (time, orientation)
      ...
      .def(nb::init<const mc_rbdyn::Robots &, unsigned int, const std::string &, double, double, double,
                    const sva::PTransformd &, const waypoints_t &,
                    const std::vector<std::pair<double, Eigen::Matrix3d>> &>(),
           "robots"_a, "robotIndex"_a, "surfaceName"_a, "duration"_a, "stiffness"_a, "weight"_a, "target"_a,
           "posWp"_a = waypoints_t{}, "oriWp"_a = std::vector<std::pair<double, Eigen::Matrix3d>>{},
           R"(
Creates a trajectory that follows a B-spline curve.
...
oriWp : list of (float, Matrix3d) pairs, optional
    Waypoints in orientation specified as pairs of (time, orientation)
      ...
      .def("spline", nb::overload_cast<>(&BSplineTrajectoryTask::spline, nb::const_), nb::rv_policy::reference_internal,
           R"(Get const reference to the underlying B-spline.)")

      // Set position waypoints
      .def("posWaypoints", &BSplineTrajectoryTask::posWaypoints, "posWp"_a,
           R"(
