...
struct PyTransformTask : public mc_tasks::TransformTask
{
  ...
  void targetVel(const sva::MotionVecd & worldVel) override { NB_OVERRIDE(targetVel, worldVel); }
};
...
weight : float, optional
    Task weight (default 500.0).
      ...
      .def(nb::init<const std::string &, const mc_rbdyn::Robots &, unsigned int, double, double>(), "surfaceName"_a,
...
weight : float, optional
    Task weight (default 500.0).
      ...
      .def("target", nb::overload_cast<const sva::PTransformd &>(&TransformTask::target), "worldPos"_a,
           R"(Set the task's target pose in the world frame.)")

      .def("targetSurface", &TransformTask::targetSurface, "robotIndex"_a, "surfaceName"_a, "offset"_a,
           R"(Target a robot surface with an optional offset.)")

      .def("targetFrame", &TransformTask::targetFrame, "targetFrame"_a, "offset"_a,
           R"(Target a given frame with an optional offset.)")

      .def("targetFrameVelocity", &TransformTask::targetFrameVelocity, "targetFrame"_a, "offset"_a,
           R"(Target a given frame velocity with an optional offset.)")

      // Properties
      .def_prop_ro("surface", &TransformTask::surface, R"(Retrieve the controlled frame name.)")

      .def_prop_ro("frame", &TransformTask::frame, R"(Return the controlled frame (mc_rbdyn.RobotFrame).)")
      ...
      .def("setGains", nb::overload_cast<const sva::MotionVecd &, const sva::MotionVecd &>(&TransformTask::setGains),
           "stiffness"_a, "damping"_a, R"(Set dimensional stiffness and damping.)")

      .def("stiffness", nb::overload_cast<const sva::MotionVecd &>(&TransformTask::stiffness), "stiffness"_a,
           R"(Set dimensional stiffness. Damping is automatically 2*sqrt(stiffness).)")

      .def("mvStiffness", &TransformTask::mvStiffness, R"(Get dimensional stiffness as a MotionVecd.)")

      .def("damping", nb::overload_cast<const sva::MotionVecd &>(&TransformTask::damping), "damping"_a,
           R"(Set dimensional damping.)")

      .def("mvDamping", &TransformTask::mvDamping, R"(Get dimensional damping as a MotionVecd.)")

      // Reference velocity / acceleration
      // Setter: refVelB(velB)
      .def("refVelB", nb::overload_cast<const sva::MotionVecd &>(&TransformTask::refVelB), "velB"_a,
           R"(Set the task's reference velocity in frame coordinates.)")

      // Getter: refVelB()
      .def("refVelB", nb::overload_cast<>(&TransformTask::refVelB, nb::const_),
      ...
      .def("load", &TransformTask::load, "solver"_a, "config"_a, R"(Load task parameters from a configuration object.)")

      .def("addToLogger", &TransformTask::addToLogger, R"(Add the task data to the logger.)");
}
