...
class TreeView(object):
    ...
    def simplify(self):
        ...
        for leaf in self.leafs:
            leaf.simplify()
        data_leafs = [leaf for leaf in self.leafs if leaf.hasData]
        numeric_leafs = [leaf for leaf in data_leafs if re.match("^[0-9]+$", leaf.name)]
        if len(data_leafs) == len(numeric_leafs) and len(numeric_leafs) > 9:
            numeric_leafs.sort(key=lambda x: int(x.name))
            self.leafs = numeric_leafs + [
                leaf for leaf in self.leafs if not leaf.hasData
            ]
    ...
    def update_y_selector(self, ySelector, parent, baseModelIdx=None):
        ...
        for leaf in self.leafs:
            leaf.widgets.append(
                MCLogTreeWidgetItem(parent, leaf.name, leaf.dataName, leaf.hasData)
            )
            if baseModelIdx is not None:
                leaf.modelIdxs.append(ySelector.model().index(row, 0, baseModelIdx))
            else:
                leaf.modelIdxs.append(ySelector.model().index(row, 0))
            leaf.update_y_selector(ySelector, leaf.widgets[-1], leaf.modelIdxs[-1])
            row += 1
    ...
    def select(self, name, ySelector, idx, fullName=""):
        if name == fullName:
            ...
            for leaf in self.leafs:
                if len(fullName):
                    fName = fullName + "_" + leaf.name
                else:
                    fName = leaf.name
                if name.startswith(fName):
                    leaf.select(name, ySelector, idx, fName)
...
class SpecialPlot(object):
    ...
    def __add_diff(self):
        added = list(
            filter(
                lambda x: re.match("{}($|_.*$)".format(self.name), x) is not None,
    ...
    def __add_rpy(self):
        ...
        if add_fn(self.figure.x_data, self.name):
            self.added = [
                "{}_{}".format(self.name, s) for s in ["roll", "pitch", "yaw"]
            ]
    ...
    def __add_rpy_diff(self):
        ...
        if add_fn(self.figure.x_data, self.name):
            self.added = [
                "{}_{}".format(self.name, s)
                for s in ["roll diff", "pitch diff", "yaw diff"]
            ]
...
class RemoveSpecialPlotButton(SpecialPlot, QtWidgets.QPushButton):
    def __init__(self, name, logtab, idx, special_id, label=None):
        ...
        QtWidgets.QPushButton.__init__(
            self, "Remove {} plot".format(self.label), logtab
        )
...
class RemoveXYDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent, remove_cb, style_cb, get_plots_cb):
        ...
        for itm in self.get_plots().keys():
            ...
            btn.released.connect(lambda label=label, b=btn: self.removePlot(label, b))
        ...
        if item.isSelected():
            if not item.hasData:
                childs = [
                    ...
                    if k.startswith("{}_".format(item.actualText))
                ]
                childs.sort()
                if len(childs) == len(self.labels) and all(
                    [len(label.text()) == 0 for label in self.labels]
                ):
                ...
                if self.nextLabel < len(self.labels):
                    ...
                    while (
                        self.nextLabel < len(self.labels)
                        and len(self.labels[self.nextLabel].text()) != 0
                    ):
                        self.nextLabel += 1
                else:
                    item.setSelected(False)
...
class XYSelector(QtWidgets.QWidget):
    def __init__(
        self, parent, add_plot_cb, remove_plot_cb, style_cb, draw_cb, get_plots_cb
    ):
        ...
        self.addButton.released.connect(
            lambda: self.dialog(self, self.parent().data).exec_()
        )
        self.removeButton = QtWidgets.QPushButton("Remove XY plot(s)")
        self.removeButton.released.connect(
            lambda: RemoveXYDialog(
                self, self.remove_plot, self.style, self.get_plots
            ).exec_()
        )
...
class XYZSelector(XYSelector):
    def __init__(
        ...
        super(XYZSelector, self).__init__(
            parent, add_plot_cb, remove_plot_cb, style_cb, draw_cb, get_plots_cb
        )
...
class MCLogTab(QtWidgets.QWidget):
    ...
    def __init__(self, parent=None, type_=PlotType.TIME):
        ...
        self.XYCanvas.hide()
        self.XYSelector1 = XYSelector(
            self,
            self.XYCanvas.add_plot_left_xy,
            self.XYCanvas.remove_plot_left,
            self.XYCanvas.style_left,
            self.XYCanvas.draw,
            lambda: self.XYCanvas._left().plots,
        )
        self.ui.y1SelectorLayout.addWidget(self.XYSelector1)
        self.XYSelector2 = XYSelector(
            self,
            self.XYCanvas.add_plot_right_xy,
            self.XYCanvas.remove_plot_right,
            self.XYCanvas.style_right,
            self.XYCanvas.draw,
            lambda: self.XYCanvas._right().plots,
        )
        ...
        self._3DCanvas.hide()
        self.XYZSelector1 = XYZSelector(
            self,
            self._3DCanvas.add_plot_left_xyz,
            self._3DCanvas.remove_plot_left,
            self._3DCanvas.style_left,
            self._3DCanvas.draw,
            lambda: self._3DCanvas._left().plots,
        )
    ...
    def setRobotModule(self, rm, filtered_joints_prefix, loaded_files):
        ...
        def findQList(ySelector):
            ...
            for qPrefix in filtered_joints_prefix:
                qList += ySelector.findItems(
                    qPrefix, QtCore.Qt.MatchStartsWith | QtCore.Qt.MatchRecursive
                )
            return qList
    ...
    def on_y1Selector_itemClicked(self, item, col):
        self.y1Selected, self.y1SelectedItems = self.itemSelectionChanged(
            self.ui.y1Selector, self.y1Selected, self.y1SelectedItems, 0
        )
    ...
    def itemSelectionChanged(self, ySelector, prevSelected, prevSelectedItems, idx):
        ...
        to_be_added = [
            (s, l) for s, l in zip(selected, legends) if s not in prevSelected
        ]
        if len(to_be_added) > 7:
            reply = QtWidgets.QMessageBox.question(
                self,
                "Warning",
                "You are trying to add many ({}) entries at once. Are you sure?".format(
                    len(to_be_added)
                ),
    ...
    def showCustomMenu(self, ySelector, point, idx):
        ...
        action = QtWidgets.QAction("Plot diff", menu)
        action.triggered.connect(
            lambda: RemoveSpecialPlotButton(
                item.actualText, self, idx, "diff", item.legendText + "_diff"
            )
        )
        ...
        if s is not None:
            for item_label, axis_label in rpy_specials:
                action = QtWidgets.QAction("Plot {}".format(item_label), menu)
                action.triggered.connect(
                    lambda checked, label=axis_label: RemoveSpecialPlotButton(
                        s.group(1), self, idx, label
                    )
                )
                menu.addAction(action)
            for item_label, axis_label in rpy_specials:
                action = QtWidgets.QAction("Plot {} diff".format(item_label), menu)
                action.triggered.connect(
                    lambda checked, label=axis_label: RemoveSpecialPlotButton(
                        s.group(1), self, idx, label + "_diff"
                    )
                )
                menu.addAction(action)
        else:
            quat_childs = filter(
                ...
                [
                    re.match("{}((_.+)*)_q?w$".format(item.actualText), x)
            ...
            for qc in quat_childs:
                for item_label, axis_label in rpy_specials:
                    if len(qc.group(1)):
                        action_text = "Plot {} {}".format(qc.group(1)[1:], item_label)
                    else:
                        action_text = "Plot {}".format(item_label)
                ...
                for item_label, axis_label in rpy_specials:
                    if len(qc.group(1)):
                        action_text = "Plot {} {} diff".format(
                            qc.group(1)[1:], item_label
                        )
                    else:
                        action_text = "Plot {} diff".format(item_label)
                    action = QtWidgets.QAction(action_text, menu)
                    plot_name = item.actualText + qc.group(1)
                    action.triggered.connect(
                        lambda checked, name=plot_name, label=axis_label: RemoveSpecialPlotButton(
    ...
    def MakeFigure(type_, data, x, y1, y2, y1_label=None, y2_label=None, figure=None):
        ...
        if y1_label is None:
            return MCLogTab.MakeFigure(
                type_, data, x, y1, y2, labels(y1), y2_label, figure
            )
        if y2_label is None:
            return MCLogTab.MakeFigure(
                type_, data, x, y1, y2, y1_label, labels(y2), figure
            )
        if figure is None:
            return MCLogTab.MakeFigure(
                type_, data, x, y1, y2, y1_label, y2_label, PlotFigure()
            )
        figure.setData(data)
        if type_ is PlotType.TIME:
    ...
    def MakePlot(parent, type_, x_data, y1, y2, y1_label=None, y2_label=None):
        ...
        if y1_label is None:
            return MCLogTab.MakePlot(
                parent, type_, x_data, y1, y2, labels(y1), y2_label
            )
        if y2_label is None:
            return MCLogTab.MakePlot(
                parent, type_, x_data, y1, y2, y1_label, labels(y2)
            )
        ...
        MCLogTab.MakeFigure(
            type_, parent.data, x_data, y1, y2, y1_label, y2_label, tab.activeCanvas
        )
    ...
    def UserFigure(data, p, figure=None, special=None):
        if figure is None:
            return MCLogTab.UserFigure(
                data, p, MCLogTab.MakeFigure(p.type, data, p.x, p.y1, p.y2), special
            )
        if special is None:
            return MCLogTab.UserFigure(
                data, p, figure, lambda y, idx, id_: UserPlot(y, figure, idx, id_)
            )
    ...
    def UserPlot(parent, p):
        ...
        MCLogTab.UserFigure(
            ...
            p,
            tab.activeCanvas,
            lambda y, idx, id_: RemoveSpecialPlotButton(y, tab, idx, id_),
        )
    ...
    def ForceSensorPlot(parent, fs):
        tab = MCLogTab.MakePlot(
            parent,
            PlotType.TIME,
            "t",
            ["{}ForceSensor_f{}".format(fs, ax) for ax in ["x", "y", "z"]],
            ["{}ForceSensor_c{}".format(fs, ax) for ax in ["x", "y", "z"]],
        )
