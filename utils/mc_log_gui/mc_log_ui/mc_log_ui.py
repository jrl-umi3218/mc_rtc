...
def read_flat(f, tmp=False):
    def read_size(fd):
        return ctypes.c_size_t.from_buffer_copy(
            fd.read(ctypes.sizeof(ctypes.c_size_t))
        ).value

    def read_bool(fd):
        return ctypes.c_bool.from_buffer_copy(
            fd.read(ctypes.sizeof(ctypes.c_bool))
        ).value
    ...
    with open(fpath) as f:
        ...
        for i, plt in enumerate(userPlotList):
            ...
            if not isinstance(plt.graph_labels, GraphLabels):
                ...
                userPlotList[i] = plt._replace(
                    graph_labels=GraphLabels(**plt.graph_labels)
                )
                plt = userPlotList[i]
...
class CommonStyleDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent, name, canvas, style):
        ...
        self.colorButton.setStyleSheet(
            "background-color: {}; color: {}".format(self.style.color, self.style.color)
        )
...
class ColorButtonRightClick(QtWidgets.QPushButton):
    ...
    def __init__(self, parent, color):
        ...
        self.setStyleSheet(
            "background-color: {color}; color: {color}".format(color=color)
        )
...
class ColorsSchemeConfigurationDialog(QtWidgets.QDialog):
    ...
    def removeColorFromSelector(self, btn):
        ...
        while itm:
            if (
                isinstance(itm.widget(), ColorButtonRightClick)
                and itm.widget() is not btn
            ):
...
class AllLineStyleDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent, name, canvas, plots, style_fn):
        ...
        def makeColorButton(self, style):
            ...
            ret.setStyleSheet(
                "background-color: {color}; color: {color}".format(color=style.color)
            )
    ...
    def __init__(self, parent):
        ...
        if len(parent.loaded_files) == 0:
            QtWidgets.QMessageBox.critical(
                self, "No log opened", "Load a log file before using this function"
            )
            self._rejected = True
        ...
        if not self.fin.endswith(".bin"):
            QtWidgets.QMessageBox.critical(
                self,
                "Invalid log format",
                "Extraction is only supported with .bin files",
            )
            self._rejected = True
            super(ExtractSelectionDialog, self).reject()
            return
        if len(parent.loaded_files) > 1:
            QtWidgets.QMessageBox.warning(
                self,
                "Extract from multiple logs not supported",
                "Multiple logs are open, only data from {} will be extract".format,
            )

        self.available_keys = []
        # Call mc_bin_utils show to extract the available keys
        show_out = subprocess.check_output(
            "mc_bin_utils show {}".format(self.fin).split()
        ).decode("ascii")
        ...
        self.selectionListWidget.setSelectionMode(
            QtWidgets.QAbstractItemView.ExtendedSelection
        )
        for k in self.available_keys:
            self.selectionListWidget.addItem(k)
            if k in selection:
                item = self.selectionListWidget.item(
                    self.selectionListWidget.count() - 1
                )
                item.setSelected(True)
        self.selectionListWidget.setMinimumWidth(
            self.selectionListWidget.sizeHintForColumn(0) + 30
        )
    ...
    def accept(self):
        ...
        if len(selection) == 0:
            QtWidgets.QMessageBox.critical(
                ...
                "You must select log entries before extracting",
            )
            return
        if os.path.exists(fout):
            overwrite = QtWidgets.QMessageBox.question(
                ...
                "{} already exists, do you want to overwrite it?".format(fout),
                QtWidgets.QMessageBox.Yes,
                QtWidgets.QMessageBox.No,
            )
            if overwrite == QtWidgets.QMessageBox.No:
                return
        cmd = "mc_bin_utils extract {} {} --keys {}".format(
            self.fin, fout, " ".join(selection)
        )
        ...
        with open(fout, "w") as fd:
            ...
            for i in i_range:
                ...
                if i == i_range[-1]:
                    ...
                    next_q = np.array(
                        [data["qOut_{}".format(jIdx)][i + 1] for jIdx in rjo_range]
                    )
...
class LabelsTitleEditDialog(QtWidgets.QDialog):
    ...
    def __init__(self, parent, canvas):
        ...
        self.extraLayout.addWidget(
            QtWidgets.QLabel("Labels legend size"), extraRow, 0, 1, 2
        )
        self.extraLayout.addWidget(
            QtWidgets.QLabel("Labels legend top offset"), extraRow, 2, 1, 1
        )
        extraRow += 1

        self.labelsLegendSizeEdit = QtWidgets.QLineEdit(
            str(canvas.labels_legend_fontsize())
        )
        self.labelsLegendSizeEdit.setValidator(QtGui.QDoubleValidator(1, 1e6, 1))
        self.labelsLegendTopOffsetEdit = QtWidgets.QLineEdit(
            str(canvas.labels_legend_top_offset())
        )
    ...
    def apply(self):
        ...
        self.canvas.labels_legend_top_offset(
            float(self.labelsLegendTopOffsetEdit.text())
        )
        self.canvas.draw()
...
class MCLogUI(QtWidgets.QMainWindow):
    def __init__(self, parent=None):
        ...
        self.gridStyles = {
            "left": LineStyle(linestyle="--"),
            "right": LineStyle(linestyle=":"),
        }
        self.gridStyleFile = (
            os.path.expanduser("~") + "/.config/mc_log_ui/grid_style.json"
        )
        ...
        self.userPlotFile = (
            os.path.expanduser("~") + "/.config/mc_log_ui/custom_plot.json"
        )
        ...
        self.polyColorsFile = (
            os.path.expanduser("~") + "/.config/mc_log_ui/poly_colors.json"
        )
        self.polyColorsScheme = ColorsSchemeConfiguration(
            self.polyColorsFile, "Pastel1"
        )
        ...
        if mc_rbdyn is not None:
            ...
            for r in mc_rbdyn.RobotLoader.available_robots():
                ...
                if "/" in r:
                    ...
                    if category not in rCategoryMenu:
        ...
        def fillLineStyleMenu(self):
            ...
            def makePlotMenu(self, name, plots, style_fn):
                ...
                action.triggered.connect(
                    lambda: AllLineStyleDialog(
                        self, name, self.getCanvas(), plots, style_fn
                    ).exec_()
                )
                ...
                for y in plots:
                    ...
                    action.triggered.connect(
                        lambda checked, yin=y, stylein=style: LineStyleDialog(
            ...
            if canvas._right():
                makePlotMenu(
                    self, "Right", canvas._right().plots.keys(), tab.style_right
                )
        ...
        self.leftGridAction.triggered.connect(
            lambda: GridStyleDialog(
                self, "left", self.getCanvas(), self.getCanvas()._left().grid
            ).exec_()
        )
        ...
        self.rightGridAction.triggered.connect(
            lambda: GridStyleDialog(
                self, "right", self.getCanvas(), self.getCanvas()._right().grid
            ).exec_()
        )
        ...
        self.colorSchemeAction.triggered.connect(
            lambda: ColorsSchemeConfigurationDialog(
                self, self.colorsScheme, self.setColorsScheme
            ).exec_()
        )
        self.styleMenu.addAction(self.colorSchemeAction)

        # Polygon color scheme selector
        self.polyColorSchemeAction = QtWidgets.QAction(
            "Polygons colors selection", self.styleMenu
        )
        self.polyColorSchemeAction.triggered.connect(
            lambda: ColorsSchemeConfigurationDialog(
                self, self.polyColorsScheme, self.setPolyColorsScheme
            ).exec_()
        )
    ...
    def saveUserPlots(self):
        ...
        with open(self.userPlotFile, "w") as f:
            json.dump(
                self.userPlotList, f, default=default_, indent=2, separators=(",", ": ")
            )
        self.update_userplot_menu()
    ...
    def update_userplot_menu(self):
        ...
        if len(self.userPlotList):
            rmUserPlotMenu = QtWidgets.QMenu(
                "Remove saved plots", self.ui.menuUserPlots
            )
            for p in self.userPlotList:
                act = QtWidgets.QAction(p.title, self.ui.menuUserPlots)
                act.triggered.connect(
                    lambda checked, plot=p: self.remove_userplot(plot)
                )
    ...
    def save_userplot(self):
        ...
        title, ok = QtWidgets.QInputDialog.getText(
            self, "User plot", "Title of your plot:", text=defaultTitle
        )
        if ok:
            ...
            if type_ is PlotType.TIME:
                y1 = [
                    convertJointIndex(x)
                    for x in filter(
                        lambda k: convertJointIndex(k) in self.data.keys(),
                        canvas._left().plots.keys(),
                    )
                ]
                y2 = [
                    convertJointIndex(x)
                ...
                y2d = []
            style = {
                convertJointIndex(y): canvas.style_left(y)
                for y in canvas._left().plots.keys()
            }
            if canvas._right():
                style2 = {
                    convertJointIndex(y): canvas.style_right(y)
                    for y in canvas._right().plots.keys()
                }
            else:
                style2 = {}
            ...
            up = UserPlot(
                ...
                style2,
                GraphLabels(
                    title=TextWithFontSize(canvas.title(), canvas.title_fontsize()),
    ...
    def plot_userplot(self, p):
        if p.type is PlotType.TIME:
            ...
            valid = p.x in self.data.keys() and all(
                [
                    x in self.data.keys() and y in self.data.keys()
                    for x, y, l in p.y1 + p.y2
                ]
            )
        else:
            valid = p.x in self.data.keys() and all(
                [
                    x in self.data.keys()
                    and y in self.data.keys()
                    and z in self.data.keys()
                    for x, y, z, l in p.y1 + p.y2
                ]
            )
        if not valid:
            missing_entries = ""
            ...
            err_diag.setModal(True)
            err_diag.setText(
                "Plot {} is not valid for this log file, some data is missing\nMissing entries:\n{}".format(
    ...
    def updateClosable(self):
        ...
        if has_closable:
            self.ui.tabWidget.tabBar().tabButton(
                self.ui.tabWidget.count() - 1, QtWidgets.QTabBar.RightSide
            ).hide()
    ...
    def load_csv(self, fpath, clear=True):
        ...
        if "t" not in data:
            print(
                "This GUI assumes a time-entry named t is available in the log, failed loading {}".format(
                    fpath
                )
            )
            return
        if "t" in self.data and not clear:
            ...
            if abs(dt - ndt) > 1e-9:
                print(
                    "This GUI assumes you are comparing logs with a similar timestep, already loaded dt = {} but attempted to load dt = {} from {}",
                    dt,
                    ndt,
                    fpath,
                )
        ...
        if "perf_SolverBuildAndSolve" in data and "perf_SolverSolve" in data:
            data["perf_SolverBuild"] = (
                data["perf_SolverBuildAndSolve"] - data["perf_SolverSolve"]
            )
        if len(self.loaded_files) > 1:
            ...
            if pad_left > 0 or pad_right > 0:
                ...
                self.data.data = {
                    k: np.concatenate(
                        ([float("nan")] * pleft, v, [float("nan")] * pright)
                    )
            ...
            for k in keys:
                ...
                self.data["{}_{}".format(fpath, k)] = np.concatenate(
                    ([float("nan")] * pleft, data[k], [float("nan")] * pright)
                )
    ...
    def update_menu(self):
        ...
        def validEntry(y):
            return any(
                [
                    y is None or (y is not None and k.startswith(y))
        ...
        menuEntries = [
            ...
            if all([validEntry(y) for y in [y1, y2, y1d, y2d]])
        ]
        for n, y1, y2, y1d, y2d in menuEntries:
    ...
    def plot_force_sensor(self, fs):
        ...
        self.ui.tabWidget.insertTab(
            self.ui.tabWidget.count() - 1, plotW, "FS: {}".format(fs)
        )
        self.ui.tabWidget.setCurrentIndex(self.ui.tabWidget.count() - 2)
        self.updateClosable()
