                                         ...
                                         const std::optional<std::string> & publishName)
{
  ...
  else if(surface.type() == "planar")
  {
    ...
    publish_surface(mc_rtc::gui::Polygon(
        publishName.value_or(name), mc_rtc::gui::LineConfig{mc_rtc::gui::Color::Green, 0.05},
    ...
    publish_surface(mc_rtc::gui::Arrow(
        publishName.value_or(name) + "_normal", mc_rtc::gui::ArrowConfig(mc_rtc::gui::Color::Blue),
        [get_pose]() { return get_pose().translation(); },
  ...
  else if(surface.type() == "gripper")
  {
    ...
    size_t pid = 0;
    mc_rtc::gui::ArrowConfig cfg{mc_rtc::gui::Color::Blue};
    for(const auto & p : gripper.pointsFromOrigin())
    {
      ...
      publish_surface(
          mc_rtc::gui::Arrow(publishName.value_or(name) + "_" + std::to_string(pid++), cfg, get_start, get_end));
    }
  }
