...
{
  if(delay == 0) { createState(ctl); }
}

bool ParallelState::DelayedState::run(Controller & ctl, double time)
{
  if(state_) { return state_->run(ctl); }
  if(time > delay_) { createState(ctl); }
  return false;
}
...
void ParallelState::start(Controller & ctl)
{
  ...
  if(states.size() == 0) { mc_rtc::log::error_and_throw("ParallelState requires at least one state to run"); }
  for(const auto & s : states)
  {
    if(!ctl.factory().hasState(s)) { mc_rtc::log::error_and_throw("{}: {} is not available", name(), s); }
  }
...
{
  bool ret = true;
  for(auto & s : states_) { ret = s.run(ctl, time_) && ret; }
  time_ += ctl.solver().dt();
  if(ret && !finished_first_)
  {
    finished_first_ = true;
    std::string out = "";
    if(outputStates_.empty()) { out = states_.back().state()->output(); }
    else
    {
      for(auto & s : states_)
      {
...
void ParallelState::teardown(Controller & ctl)
{
  for(auto & s : states_)
  {
    if(s.state()) { s.state()->teardown_(ctl); }
  }
}
...
bool ParallelState::read_msg(std::string & msg)
{
  for(auto & s : states_)
  {
    if(s.state() && s.state()->read_msg(msg)) { return true; }
  }
  return false;
}
...
bool ParallelState::read_write_msg(std::string & msg, std::string & out)
{
  for(auto & s : states_)
  {
    if(s.state() && s.state()->read_write_msg(msg, out)) { return true; }
  }
  return false;
}
