...
void CoMTrajectoryGeneration_Initial::start(mc_control::fsm::Controller & ctl)
{
  ...
  //clang-format off
  steps_.reset({{0.0, {currCoM.x(), currCoM.y()}},
                {0.2, {0.0, centerY}},
                {2, {0.0, centerY}},
                {2.2, {0.0, rightY}},
                {4, {0.0, rightY}},
  ...
  comGenerator_ = std::make_shared<mc_planning::generator>(preview_, ctl.robot().mass(), waist_height);
  ...
  ctl.gui()->addPlot(
      "Trajectory (Y)", mc_rtc::gui::plot::X("t", [this]() { return t_; }),
      mc_rtc::gui::plot::Y("Output CoM", [this]() { return comGenerator_->OutputCOGPosition().y(); }, Color::Magenta),
      mc_rtc::gui::plot::Y("Ideal  CoM", [this]() { return comGenerator_->IdealCOGPosition().y(); }, Color::Red,
                           Style::Dashed),
      mc_rtc::gui::plot::Y("Output ZMP", [this]() { return comGenerator_->OutputZMPPosition().y(); }, Color::Purple),
      mc_rtc::gui::plot::Y("Ideal  ZMP", [this]() { return comGenerator_->IdealZMPPosition().y(); }, Color::Blue,
                           Style::Dashed));

  ctl.gui()->addElement(
      {"CoMTrajectoryGeneration"},
      mc_rtc::gui::NumberInput("Delay", [this]() { return delay_; }, [this](double delay) { delay_ = delay; }),
      mc_rtc::gui::NumberInput("Transition time", [this]() { return transition_; },
                               [this](double transition) { transition_ = transition; }),
      mc_rtc::gui::Button("Left",
                          [this, leftY]() {
                            auto curr = t_ + previewTime_ + delay_;
                            comGenerator_->changeFutureSteps(curr, {{curr + transition_, {0, leftY}}});
                            updateSteps();
                          }),
      mc_rtc::gui::Button("Right",
                          [this, rightY]() {
                            auto curr = t_ + previewTime_ + delay_;
                            comGenerator_->changeFutureSteps(curr, {{curr + transition_, {0, rightY}}});
                            updateSteps();
                          }),
      mc_rtc::gui::Button("Center",
                          [this, centerY]() {
                            auto curr = t_ + previewTime_ + delay_;
                            comGenerator_->changeFutureSteps(curr, {{curr + transition_, {0, centerY}}});
                            updateSteps();
                          }),
      mc_rtc::gui::Button("Left-Right-Left", [this, centerY, leftY, rightY]() {
        auto curr = t_ + previewTime_ + delay_;
  ...
  ctl.logger().addLogEntry("perf_CoMGenerator[ms]", [this]() { return generationTime_; });
}
...
bool CoMTrajectoryGeneration_Initial::run(mc_control::fsm::Controller & ctl)
{
   ...
   */
  auto generateTrajectory = [this]() { comGenerator_->generate(iter_); };
  ...
  {
    ctl.datastore().call("StabilizerStandingTrackCoM::target", comGenerator_->OutputCOGPosition(),
                         comGenerator_->OutputCOGVelocity(), comGenerator_->OutputCOGAcceleration(),
                         comGenerator_->OutputZMPPosition());
  }
