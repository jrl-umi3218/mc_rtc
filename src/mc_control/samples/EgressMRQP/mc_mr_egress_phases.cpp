...
public:
  ...
  : started(false), done_move_foot(false), done_change_knee(false), done_rotate(false), done_reorient(false),
    done_putdown(false), forceIter(0), forceStart(0)
  {
  }
  ...
  virtual bool run(MCEgressMRQPController & ctl) override
  {
    ...
    {
      timeoutIter++;
      if((ctl.efTask->positionTask->eval().norm() < 1e-1 && ctl.efTask->positionTask->speed().norm() < 1e-4)
         || timeoutIter > 10 * 500)
      {
        done_move_foot = true;
    ...
    else if(!done_rotate)
    {
      ...
      timeoutIter++;
      if((ctl.efTask->positionTask->eval().norm() < 1e-1 && ctl.efTask->positionTask->speed().norm() < 1e-4
          && ctl.efTask->orientationTask->eval().norm() < 1e-2 && ctl.efTask->orientationTask->speed().norm() < 1e-4)
         || timeoutIter > 15 * 500)
      {
    ...
    else if(!done_putdown)
    {
      ...
      if(forceIter > 40 || timeoutIter > 15 * 500)
      {
        ...
        ctl.egressContacts.push_back(mc_rbdyn::Contact(ctl.robots(), 0, 1, "RFullSole", "left_floor"));
...
public:
  ...
  : started(false), done_removing(false), done_rotating(false), done_contacting(false), lfc_index(0), forceIter(0),
    forceStart(0), com_multiplier(0.1), otherContacts()
  {
  }
  ...
  virtual bool run(MCEgressMRQPController & ctl) override
  {
    if(!started)
    {
      ...
      ctl.solver().removeTask(ctl.efTask);
      ctl.efTask.reset(new mc_tasks::EndEffectorTask("LLEG_LINK5", ctl.robots(), ctl.robots().robotIndex(), 0.25));
    ...
    else
    {
      ...
      {
        timeoutIter++;
        if((ctl.efTask->positionTask->eval().norm() < 1e-2 && ctl.efTask->positionTask->speed().norm() < 1e-4)
           || timeoutIter > 15 * 500)
        {
          done_removing = true;
      ...
      else if(!done_contacting)
      {
        ...
        if(forceIter > 40 || timeoutIter > 15 * 500)
        {
          ...
          ctl.egressContacts.emplace_back(ctl.robots(), ctl.robots().robotIndex(), 2, "LFullSole", "AllGround");
          ctl.solver().setContacts(ctl.egressContacts);
...
public:
  ...
  : started(false), done_removing(false), done_moving(false), done_rotating(false), done_contacting(false),
    forceIter(0), forceStart(0), otherContacts()
  {
  }
  ...
  virtual bool run(MCEgressMRQPController & ctl) override
  {
    if(!started)
    {
      ...
      ctl.solver().removeTask(ctl.efTask);
      ctl.efTask.reset(new mc_tasks::EndEffectorTask("RLEG_LINK5", ctl.robots(), ctl.robots().robotIndex(), 0.25));
      ...
      // Free movement along z axis
      auto rfc =
          std::find_if(ctl.egressContacts.begin(), ctl.egressContacts.end(), [](const mc_rbdyn::Contact & c) -> bool {
            return c.r1Surface()->name().compare("RFullSole") == 0;
          });
    ...
    else
    {
      ...
      {
        timeoutIter++;
        if((ctl.efTask->orientationTask->eval().norm() < 1e-2 && ctl.efTask->orientationTask->speed().norm() < 1e-4)
           || timeoutIter > 15 * 500)
        {
          done_rotating = true;
          unsigned int rfindex = ctl.robot().bodyIndexByName("RLEG_LINK5");
      ...
      else if(!done_contacting)
      {
        ...
        if(forceIter > 40 || timeoutIter > 15 * 500)
        {
          ...
          constr->resetDofContacts();
          // NB : When using dof contacts, do !add twice !
          ctl.egressContacts.emplace_back(ctl.robots(), ctl.robots().robotIndex(), 2, "RFullSole", "AllGround");
          ctl.solver().setContacts(ctl.egressContacts);
...
public:
  ...
  : started(false), done_removing(false), done_premoving(false), done_moving(false), done_rotating(false),
    done_contacting(false), rfc_index(2), forceIter(0), forceStart(0), otherContacts()
  {
  }
    ...
    else
    {
      if(!done_premoving)
      {
           ...
           || timeoutIter > 15 * 500)
        {
          done_premoving = true;
          ctl.solver().setContacts(otherContacts);
          Eigen::Vector3d move(-0.14, 0.0, 0); /*FIXME For safer egress, this should be based on the relative position
                                                  between the right && the left foot */
      ...
      if(!done_removing)
      {
           ...
           || timeoutIter > 15 * 500)
        {
          done_removing = true;
          // ctl.solver().setContacts(otherContacts);
          Eigen::Vector3d move(0., 0.3, 0); /*FIXME For safer egress, this should be based on the relative position
                                               between the right && the left foot */
          ctl.efTask->positionTask->position(ctl.efTask->positionTask->position() + move);
          timeoutIter = 0;
          // ctl.solver().removeTask(ctl.efTask);
          ...
          // ctl.trajTask.reset(new mc_tasks::TrajectoryTask(ctl.robots(), 0,
          // *(ctl.robot().surfaces.at("RFullSole").get()), target, 30.0, ctl.timeStep, 0.1, 1e2, 1));//,
          // "TrajectoryTask", wps)); ctl.solver().addTask(ctl.trajTask); prev_weight =
          // ctl.efTask->orientationTask->weight(); ctl.efTask->orientationTask->weight(0.1); auto p =
          // ctl.postureTask->posture(); int rankle_i = ctl.robot().jointIndexByName("RLEG_JOINT4"); p[rankle_i][0] =
          // -90*M_PI/180; ctl.postureTask->posture(p);
          LOG_INFO("Modified position")
        }
        return false;
      }
      ...
      else if(!done_rotating)
      {
           ...
           || timeoutIter > 15 * 500)
        {
          done_rotating = true;
          ctl.solver().removeTask(ctl.efTask);
          ctl.efTask.reset(new mc_tasks::EndEffectorTask("RLEG_LINK5", ctl.robots(), ctl.robots().robotIndex(), 0.1));
          unsigned int lfindex = ctl.robot().bodyIndexByName("RLEG_LINK5");
...
{
public:
  EgressPlaceRightFootPhase() : started(false), done_lifting(false), done_rotating(false), done_contacting(false) {}
    ...
    else
    {
      ...
      if(!done_lifting)
      {
           ...
           || timeoutIter > 15 * 500)
        {
          ...
          Eigen::Matrix3d & rot = ctl.robot().mbc().bodyPosW[lfindex].rotation();
          Eigen::Vector3d rpy = rot.eulerAngles(2, 1, 0);
          auto target = sva::RotZ(-M_PI / 2) * sva::RotY(rpy(1)) * sva::RotX(rpy(2));
          ctl.efTask->orientationTask->orientation(target);
          timeoutIter = 0;
          LOG_INFO("Modified orientation")
        }
      ...
      {
        if((ctl.efTask->orientationTask->eval().norm() < 1e-1 && ctl.efTask->orientationTask->speed().norm() < 1e-4)
           || timeoutIter > 15 * 500)
        {
          done_rotating = true;
          unsigned int lfindex = ctl.robot().bodyIndexByName("LLEG_LINK5");
      ...
      else if(!done_contacting)
      {
        ...
        {
          done_contacting = true;
          ctl.egressContacts.emplace_back(ctl.robots(), ctl.robots().robotIndex(), 1, "RFullSole", "exit_platform");
          ctl.solver().setContacts(ctl.egressContacts);
          timeoutIter = 0;
          LOG_INFO("Phase finished, can transit")
          // return true;
...
{
public:
  EgressOpenRightGripperPhase() : started(false), done_opening(false) {}
...
public:
  ...
  : started(false), done_rotating(false), done_removing(false), wiggles_(0), maxWiggles_(max_wiggles), dist_(dist),
    rot(sva::RotX(deg * M_PI / 180)), birot(sva::RotX(2 * deg * M_PI / 180))
  {
  ...
  virtual bool run(MCEgressMRQPController & ctl) override
  {
    if(!started)
    {
      ...
      ctl.efTask.reset(new mc_tasks::EndEffectorTask("RARM_LINK6", ctl.robots(), ctl.robots().robotIndex(), 12., 1000));

      unsigned int rgindex = ctl.robot().bodyIndexByName("RARM_LINK6");

      ctl.efTask->orientationTask->orientation(
          (sva::PTransformd(rot) * ctl.robot().mbc().bodyPosW[rgindex]).rotation());

      // Free movement along z axis
      auto rgc =
          std::find_if(ctl.egressContacts.begin(), ctl.egressContacts.end(), [](const mc_rbdyn::Contact & c) -> bool {
            return c.r1Surface()->name().compare("RightGripper") == 0;
          });
      ctl.solver().setContacts(ctl.egressContacts);
...
{
public:
  EgressMRStandupPhase(Eigen::Vector3d offset) : started(false), done_standup(false), altitude_(offset), otherContacts()
  {
  ...
  virtual bool run(MCEgressMRQPController & ctl) override
  {
    ...
    {
      ++timeoutIter;
      if((ctl.efTask->positionTask->eval().norm() < 1e-1 && ctl.efTask->orientationTask->speed().norm() < 1e-4
...
public:
  ...
  : started(false), done_com(false), iter_(0), altitude_(altitude), surfName_(surfName), auto_transit(auto_transit)
  {
  }
  ...
  virtual bool run(MCEgressMRQPController & ctl) override
  {
    ...
    else
    {
      ...
      {
        ++iter_;
        if((ctl.comTask->eval().norm() < 1e-2 && ctl.comTask->speed().norm() < 1e-3) || iter_ > 10 * 500)
        {
          done_com = true;
          ctl.postureTask->posture(ctl.robot().mbc().q);
...
{
public:
  EgressCenterComPhase(double altitude) : started(false), done_com(false), iter_(0), altitude_(altitude) {}
  ...
  virtual bool run(MCEgressMRQPController & ctl) override
  {
    if(!started)
    {
      ...
      Eigen::Vector3d pos = (rfs.X_0_s(ctl.robot()).translation() + lfs.X_0_s(ctl.robot()).translation()) / 2;
      pos(2) = pos(2) + 0.76 + altitude_;
    ...
    else
    {
      if(!done_com)
      {
        ...
        if((ctl.comTask->eval().norm() < 1e-2 && ctl.comTask->speed().norm() < 1e-3) || iter_ > 10 * 500)
        {
          ...
          ctl.postureTask->posture(ctl.robot().mbc().q);
          // ctl.solver().removeTask(ctl.comTask);
          LOG_INFO("Centered com, error " << ctl.comTask->eval().transpose())
          // return true;
        }
        return false;
      }
...
public:
  ...
  : started(false), done_com(false), iter_(0), altitude_(altitude), comDist_(0), maxMove_(max_move), curCom(0, 0, 0),
    startPos_(0, 0, 0), surfName_(surfName)
  {
  ...
  {
    curCom = rbd::computeCoM(ctl.robot().mb(), ctl.robot().mbc());
  }
  ...
  virtual bool run(MCEgressMRQPController & ctl) override
  {
    if(!started)
    {
      LOG_INFO("Moving com over " << surfName_ << " && lifting " << otherSurf_)
      const auto & target = ctl.robot().surface(surfName_);
      Eigen::Vector3d pos = target.X_0_s(ctl.robot()).translation();
      ...
      ctl.solver().removeTask(ctl.efTask);
      ctl.efTask.reset(new mc_tasks::EndEffectorTask(bodyName_, ctl.robots(), ctl.robots().robotIndex(), 2.0, 1000));

      ctl.solver().setContacts(ctl.egressContacts);

      auto lfc =
          std::find_if(ctl.egressContacts.begin(), ctl.egressContacts.end(), [&](const mc_rbdyn::Contact & c) -> bool {
            return c.r1Surface()->name().compare(otherSurf_) == 0;
          });
      tasks::qp::ContactId cId = lfc->contactId(ctl.robots());
      Eigen::MatrixXd dof(6, 6);
      dof.setIdentity();
      ...
      {
        constr->resetDofContacts();
        LOG_INFO("Added a dof to " << lfc->r1Surface()->name() << " / " << lfc->r2Surface()->name())
        constr->addDofContact(cId, dof);
      }
...
{
public:
  EgressReplaceRightHandPhase() : started(false), done_moving(false), done_posturing(false), nrIter_(0) {}
  ...
  virtual bool run(MCEgressMRQPController & ctl) override
  {
    if(!started)
    {
      ...
      ctl.efTask.reset(new mc_tasks::EndEffectorTask("RARM_LINK6", ctl.robots(), ctl.robots().robotIndex(), 0.25));
    ...
    else
    {
      ...
      {
        ++nrIter_;
        if((ctl.efTask->eval().norm() < 1e-2 && ctl.efTask->speed().norm() < 1e-3) || nrIter_ > 10 * 500)
        {
          done_moving = true;
          ctl.solver().removeTask(ctl.efTask);
      ...
      else if(!done_posturing)
      {
        ...
        if(paramNorm(ctl.robots().mbcs()[0].alphaD) < 1e-3 || nrIter_ > 10 * 500)
        {
          done_posturing = true;
          LOG_INFO("Phase done")
        }
