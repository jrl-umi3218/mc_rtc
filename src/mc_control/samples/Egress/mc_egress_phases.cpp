...
public:
  ...
  : done_setup_lift(false), done_lift(false), done_setup_rotate(false), done_rotate(false), done_setup_putdown(false),
    done_putdown(false), iterSincePutDown(0), iterForce(0)
  {
  ...
  virtual bool run(MCEgressController & ctl) override
  {
    if(!done_lift)
    {
      if(!done_setup_lift)
      {
        ctl.solver().setContacts({mc_rbdyn::Contact(ctl.robots(), "Butthock", "left_seat"),
    ...
    else if(!done_putdown)
    {
      if(!done_setup_putdown)
      {
        ...
        ctl.efTask->set_ef_pose(
            sva::PTransformd(sva::RotZ(-M_PI / 8) * bpw.rotation(), (ctl.efTask->get_ef_pose() * down).translation()));
        iterSincePutDown = 0;
        iterForce = 0;
...
public:
  ...
  : done_setup_normal_move(false), done_normal_move(false), done_setup_move_right(false), done_move_right(false),
    done_setup_go_to_posture(false), done_go_to_posture(false), iterSinceMoving(0)
  {
  }
  virtual bool run(MCEgressController & ctl) override
  {
...
{
public:
  EgressRotateBodyPhase() : done_setup_rotate_body(false), done_rotate_body(false), timeoutIter(0) {}
  virtual bool run(MCEgressController & ctl) override
  {
    if(!done_rotate_body)
    {
...
struct EgressMoveFootOutPhase : public EgressPhaseExecution
{
  ...
  : done_setup_lift(false), done_lift(false), done_setup_reorient(false), done_reorient(false),
    done_setup_rotate(false), done_rotate(false), done_setup_putdown(false), done_putdown(false), ankle_i(0),
    ankle_reorient_target(0), timeoutIter(0)
  {
  }
...
public:
  ...
  : done_setup_lift(false), done_lift(false), done_setup_rotate(false), done_rotate(false), done_setup_putdown(false),
    done_putdown(false), iterForce(0), timeoutIter(0)
  {
  }
  virtual bool run(MCEgressController & ctl) override
  {
...
{
  EgressStandupPhase() : done_setup_standup(false), done_standup(false) {}
