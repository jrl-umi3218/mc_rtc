...
: MCController({robot_module, mc_rbdyn::RobotLoader::get_robot_module("PolarisRanger", true)}, dt),
  graspOffset(sva::RotX(-M_PI / 2), Eigen::Vector3d(0., 0., 0.)), ef_task("RARM_LINK6", robots(), 0), drivingContacts(),
  collsConstraint(robots(), 0, 1, timeStep), iter_(0), theta_(0), logging_(false), log_ankle_(), log_wheel_(),
  log_ef_(), log_acc_(), log_rpy_(), tMax_(0.5), tMin_(-0.5), head_task("HEAD_LINK1", robots(), 0),
  lhand_task("LARM_LINK6", robots(), 0)
{
  ...
  postureTask->jointStiffness(solver(), jsv);
  qpsolver->addTask(postureTask.get());

  // robot().mbc().q[0] = {0.8018680589369662, 0.09936561148509283, -0.06541812773434774, 0.5855378381237102,
  // -0.3421374123035909, -0.0002850914593993392, 0.8847053544605464};
  robot().mbc().q[0] = {1, 0, 0, 0, 0, 0, 0.76};

  rbd::forwardKinematics(robot().mb(), robot().mbc());
  rbd::forwardVelocity(robot().mb(), robot().mbc());

  drivingContacts.emplace_back(robots(), 0, 1, "Butthock", "left_seat");
  drivingContacts.emplace_back(robots(), 0, 1, "LFullSole", "exit_platform");
  drivingContacts.emplace_back(robots(), 0, 1, "LeftThight", "left_seat");
  drivingContacts.emplace_back(robots(), 0, 1, "RightThight", "left_seat");
  drivingContacts.emplace_back(robots(), 0, 1, "RightGripper", "bar_wheel");
  ...
  polarisPostureTask = std::shared_ptr<tasks::qp::PostureTask>(
      new tasks::qp::PostureTask(robots().mbs(), 1, robots().robot(1).mbc().q, 5, 100));
...
bool MCDrivingController::run()
{
  ...
  if(logging_)
  {
    ...
    log_wheel_ << t << " " << polaris.mbc().q[wheel_i][0] << " " << polarisPostureTask->posture()[wheel_i][0]
               << std::endl;
