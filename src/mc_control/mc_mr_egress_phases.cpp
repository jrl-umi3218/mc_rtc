    ...
    virtual bool run(MCEgressMRQPController & ctl) override
    {
      ...
      else if(!done_reorient)
      {
        ...
        if((ctl.efTask->positionTask->eval().norm() < 1e-1 && ctl.efTask->positionTask->speed().norm() < 1e-4 && ctl.efTask->orientationTask->eval().norm() < 1e-2 && ctl.efTask->orientationTask->speed().norm() < 1e-4) || timeoutIter > 15*500)
        {
          ...
          forceIter = 0;
          forceStart = ctl.getWrenches()[0].force()[2];
          timeoutIter = 0;
      ...
      {
        timeoutIter++;
        if(ctl.getWrenches()[0].force()[2] > forceStart + 50)
        {
      ...
      else
      {
        ...
        else if(!done_rotating)
        {
              ...
              || timeoutIter > 15*500)
          {
            ...
            timeoutIter = 0;
            forceIter = 0;
            forceStart = ctl.getWrenches()[1].force()[2];
        ...
        {
          timeoutIter++;
          if(ctl.getWrenches()[1].force()[2] > forceStart + 150)
          {
            LOG_INFO("Contact force triggered")
            forceIter++;
          }
  ...
  public:
    ...
    virtual bool run(MCEgressMRQPController & ctl) override
    {
      ...
      else
      {
        ...
        {
          timeoutIter++;
          if(ctl.getWrenches()[0].force()[2] > forceStart + 150)
          {
            LOG_INFO("Contact force triggered")
            forceIter++;
          }
