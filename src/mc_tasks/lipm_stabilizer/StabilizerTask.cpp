                                            ...
                                            const double delta)
{
  ...
  measuredLeftCoP_delayed.x() = measuredLeftCoP.x() * exp(-c_.lambdaCoP.x() * t_delay) + (1 - exp(-c_.lambdaCoP.x() * t_delay)) * delayedTargetCoPLeft_.x();
  measuredLeftCoP_delayed.y() = measuredLeftCoP.y() * exp(-c_.lambdaCoP.y() * t_delay) + (1 - exp(-c_.lambdaCoP.y() * t_delay)) * delayedTargetCoPLeft_.y();
  measuredLeftCoP_delayed.z() = measuredFzLeft * exp(-c_.lambdaCoP.z() * t_delay) + (1 - exp(-c_.lambdaCoP.z() * t_delay)) * delayedTargetFzLeft_;

  Eigen::Vector3d measuredRightCoP_delayed;
  measuredRightCoP_delayed.x() = measuredRightCoP.x() * exp(-c_.lambdaCoP.x() * t_delay) + (1 - exp(-c_.lambdaCoP.x() * t_delay)) * delayedTargetCoPRight_.x();
  measuredRightCoP_delayed.y() = measuredRightCoP.y() * exp(-c_.lambdaCoP.y() * t_delay) + (1 - exp(-c_.lambdaCoP.y() * t_delay)) * delayedTargetCoPRight_.y();
  measuredRightCoP_delayed.z() = measuredFzRight * exp(-c_.lambdaCoP.z() * t_delay) + (1 - exp(-c_.lambdaCoP.z() * t_delay)) * delayedTargetFzRight_;
