...
MoveContactTask::MoveContactTask(mc_rbdyn::Robots & robots,
                                 ...
                                 double positionWStartPercent)
: robots(robots), robot(robot), env(env), robotSurf(contact.r1Surface()), envSurf(contact.r2Surface())
{
  type_ = "move_contact";
  name_ = "move_contact_" + robot.name() + "_" + contact.r1Surface()->name() + "_" + env.name() + "_"
  ...
  set_target_tf(contact.X_0_r1s(robots), preContactDist, waypointPos, adjustOffset, adjustRPYOffset);

  posStiff = posStiffness;
  extraPosStiff = extraPosStiffness;

  positionTask.reset(new tasks::qp::PositionTask(robots.mbs(), 0, robotSurf->bodyName(),
                                                 robotSurfacePos().translation(), robotSurf->X_b_s().translation()));
  positionTaskSp.reset(
      new tasks::qp::SetPointTask(robots.mbs(), 0, positionTask.get(), posStiff, posWeight * positionWStartPercent));
  positionTaskSm.reset(new SmoothTask<Eigen::Vector3d>(
      std::bind(static_cast<void (tasks::qp::SetPointTask::*)(double)>(&tasks::qp::SetPointTask::weight),
                positionTaskSp.get(), std::placeholders::_1),
      std::bind(static_cast<double (tasks::qp::SetPointTask::*)() const>(&tasks::qp::SetPointTask::weight),
                positionTaskSp.get()),
      std::bind(
          static_cast<void (tasks::qp::PositionTask::*)(const Eigen::Vector3d &)>(&tasks::qp::PositionTask::position),
          positionTask.get(), std::placeholders::_1),
      std::bind(
          static_cast<const Eigen::Vector3d & (tasks::qp::PositionTask::*)() const>(&tasks::qp::PositionTask::position),
          positionTask.get()),
      posWeight, robotSurfacePos().translation(), 1));
  orientationTask.reset(new tasks::qp::OrientationTask(robots.mbs(), 0, robotSurf->bodyName(),
                                                       robot.mbc().bodyPosW[robotBodyIndex].rotation()));
...
{
  target(wp, targetOri, config.contactTask.position.stiffness, config.contactTask.position.extraStiffness,
         config.contactTask.position.weight, config.contactTask.orientation.stiffness,
         config.contactTask.orientation.weight, positionSmoothPercent);
}
...
{
  target(wp, targetOri, posStiffness, extraPosStiffness, posWeight, oriStiffness, oriWeight, positionSmoothPercent);
}

void MoveContactTask::toWaypoint()
{
  target(wp, targetOri, posStiff, extraPosStiff, positionTaskSm->weight, orientationTaskSp->stiffness(),
         orientationTaskSp->weight(), 1.0);
}

void MoveContactTask::toPreEnv(mc_rbdyn::StanceConfig & config, double positionSmoothPercent)
{
  target(preTargetPos, targetOri, config.contactTask.position.stiffness, config.contactTask.position.extraStiffness,
         config.contactTask.position.weight, config.contactTask.orientation.stiffness,
         config.contactTask.orientation.weight, positionSmoothPercent);
}
...
void MoveContactTask::toPreEnv(double posStiffness,
                               ...
                               double positionSmoothPercent)
{
  target(preTargetPos, targetOri, posStiffness, extraPosStiffness, posWeight, oriStiffness, oriWeight,
         positionSmoothPercent);
}

void MoveContactTask::toPreEnv()
{
  target(preTargetPos, targetOri, posStiff, extraPosStiff, positionTaskSm->weight, orientationTaskSp->stiffness(),
         orientationTaskSp->weight(), 1.0);
}

void MoveContactTask::target(const Eigen::Vector3d & pos,
...
{
  set_target_tf(X_target, config.contactObj.preContactDist, config.contactTask.waypointConf.pos,
                config.contactObj.adjustOffset, config.contactObj.adjustRPYOffset);
}
...
void MoveContactTask::selectActiveJoints(mc_solver::QPSolver & solver, const std::vector<std::string> & aJN)
{
  ...
  positionJSTask = std::make_shared<tasks::qp::JointsSelector>(
      tasks::qp::JointsSelector::ActiveJoints(robots.mbs(), 0, positionTask.get(), aJN));
  positionTaskSp = std::make_shared<tasks::qp::SetPointTask>(robots.mbs(), 0, positionJSTask.get(), positionStiff,
                                                             positionW * positionWPercent);
  positionTaskSm.reset(new SmoothTask<Eigen::Vector3d>(
      std::bind(static_cast<void (tasks::qp::SetPointTask::*)(double)>(&tasks::qp::SetPointTask::weight),
                positionTaskSp.get(), std::placeholders::_1),
      std::bind(static_cast<double (tasks::qp::SetPointTask::*)() const>(&tasks::qp::SetPointTask::weight),
  ...
  orientationJSTask = std::make_shared<tasks::qp::JointsSelector>(
      tasks::qp::JointsSelector::ActiveJoints(robots.mbs(), 0, orientationTask.get(), aJN));
  double oriStiffness = orientationTaskSp->stiffness();
  double oriWeight = orientationTaskSp->weight();
  orientationTaskSp =
      std::make_shared<tasks::qp::SetPointTask>(robots.mbs(), 0, orientationJSTask.get(), oriStiffness, oriWeight);
  orientationTaskSp =
      std::make_shared<tasks::qp::SetPointTask>(robots.mbs(), 0, orientationJSTask.get(), oriStiffness, oriWeight);
  if(putBack)
  {
...
void MoveContactTask::selectUnactiveJoints(mc_solver::QPSolver & solver, const std::vector<std::string> & uJN)
{
  bool putBack = inSolver;
  if(putBack)
  {
  ...
  double positionWPercent = 1 / static_cast<double>(positionTaskSm->nrIter);
  positionJSTask = std::make_shared<tasks::qp::JointsSelector>(
      tasks::qp::JointsSelector::UnactiveJoints(robots.mbs(), 0, positionTask.get(), uJN));
  positionTaskSp = std::make_shared<tasks::qp::SetPointTask>(robots.mbs(), 0, positionJSTask.get(), positionStiff,
                                                             positionW * positionWPercent);
  positionTaskSm.reset(new SmoothTask<Eigen::Vector3d>(
      std::bind(static_cast<void (tasks::qp::SetPointTask::*)(double)>(&tasks::qp::SetPointTask::weight),
  ...
  /* JS on orientation task */
  orientationJSTask = std::make_shared<tasks::qp::JointsSelector>(
      tasks::qp::JointsSelector::UnactiveJoints(robots.mbs(), 0, orientationTask.get(), uJN));
  double oriStiffness = orientationTaskSp->stiffness();
  double oriWeight = orientationTaskSp->weight();
  orientationTaskSp =
      std::make_shared<tasks::qp::SetPointTask>(robots.mbs(), 0, orientationJSTask.get(), oriStiffness, oriWeight);
...
void MoveContactTask::resetJointsSelector(mc_solver::QPSolver & solver)
{
  ...
  positionTaskSp = std::make_shared<tasks::qp::SetPointTask>(robots.mbs(), 0, positionTask.get(), positionStiff,
                                                             positionW * positionWPercent);
  positionTaskSm.reset(new SmoothTask<Eigen::Vector3d>(
      std::bind(static_cast<void (tasks::qp::SetPointTask::*)(double)>(&tasks::qp::SetPointTask::weight),
                positionTaskSp.get(), std::placeholders::_1),
  ...
  double oriWeight = orientationTaskSp->weight();
  orientationTaskSp =
      std::make_shared<tasks::qp::SetPointTask>(robots.mbs(), 0, orientationTask.get(), oriStiffness, oriWeight);
  orientationTaskSp =
      std::make_shared<tasks::qp::SetPointTask>(robots.mbs(), 0, orientationTask.get(), oriStiffness, oriWeight);
  if(putBack)
  {
