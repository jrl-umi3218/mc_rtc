...
int main(int argc, char **argv)
{
  ...
  // sensor thread
  HRP2OpenHRPSensors sensor_data_init;
  ...
  initTab(sensor_data_init.position, sensors_traits<HRP2OpenHRPSensors>::dof, 100);
  boost::asio::io_service io_sensor_srv;
  WriteAndAck<HRP2OpenHRPSensors> sensor_proto(io_sensor_srv, sensor_data_init);
  Server<WriteAndAck<HRP2OpenHRPSensors> > sensor_server(io_sensor_srv,
                                                         port_sensor, sensor_proto);
  boost::thread sensor_thread(srvThread, boost::ref(io_sensor_srv));

  // control thread
  HRP2OpenHRPControl control_data_init;
  boost::asio::io_service io_control_srv;
  ReadAndAck<HRP2OpenHRPControl> control_proto(io_control_srv, control_data_init);
  Server<ReadAndAck<HRP2OpenHRPControl> > control_server(io_control_srv, port_control,
                                                         control_proto);
  ...
  while(true)
  {
    ...
    printTab(sensor_data_init.position, sensors_traits<HRP2OpenHRPSensors>::dof);
    std::cout << std::endl;

    std::cout << "control: " << std::endl;
    HRP2OpenHRPControl control_data = control_proto.data();
    std::cout << "ZMP: " << control_data.hasZmp << std::endl;
    std::cout << "FF: " << control_data.hasBaseFF << std::endl;
    printTab(control_data.control, control_traits<HRP2OpenHRPControl>::dof);
    std::cout << std::endl;
