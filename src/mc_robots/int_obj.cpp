...
IntObjRobotModule::IntObjRobotModule(const std::string & env_path, const std::string & env_name)
: RobotModule(env_path, env_name)
{
  ...
  if(ifs.is_open())
  {
    ...
    urdf << ifs.rdbuf();
    mc_rbdyn_urdf::URDFParserResult res = mc_rbdyn_urdf::rbdyn_from_urdf(urdf.str(), false, {}, true, env_name);
    mb = res.mb;
    mbc = res.mbc;
    mbg = res.mbg;
