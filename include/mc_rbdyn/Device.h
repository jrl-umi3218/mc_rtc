...
struct MC_RBDYN_DLLAPI Device
{
  ...
  inline const std::string & parent() const noexcept { return parent_; }

  /** Change the parent body of the sensor */
  inline void parent(const std::string & p) noexcept { parent_ = p; }

  /** Returns the transformation from the parent body to the device */
  inline const sva::PTransformd & X_p_d() const noexcept { return X_p_s_; }

  /** Returns the transformation from the parent body to the sensor */
  inline const sva::PTransformd & X_p_s() const noexcept { return X_p_s_; }

  /** Change the parent to device transformation */
  inline void X_p_d(const sva::PTransformd & pt) noexcept { X_p_s_ = pt; }

  /** Change the parent to sensor transformation */
  inline void X_p_s(const sva::PTransformd & pt) noexcept { X_p_s_ = pt; }
