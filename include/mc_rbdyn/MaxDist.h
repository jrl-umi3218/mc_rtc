...
/** \class MaxDist
 * \brief Used to define a maximum distance constraint between two bodies
 */

namespace mc_rbdyn
{

struct MC_RBDYN_DLLAPI MaxDist
{
  MaxDist() : body1("NONE"), body2("NONE") {}
  MaxDist(const std::string & b1,
          const std::string & b2,
          double i,
          double s,
          double d,
  ...
  bool operator==(const MaxDist & rhs) const;
  bool operator!=(const MaxDist & rhs) const;
};

MC_RBDYN_DLLAPI std::ostream & operator<<(std::ostream & os, const MaxDist & c);
