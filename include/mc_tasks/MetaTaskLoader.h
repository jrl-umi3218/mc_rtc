...
struct MC_TASKS_DLLAPI MetaTaskLoader
{
  ...
  using load_fun = std::function<MetaTaskPtr(const mc_rbdyn::Robots&, const mc_rtc::Configuration&)>;
   ...
   *
   * \param robots Robots to create the task for
   *
   ...
   */
  static MetaTaskPtr load(const mc_rbdyn::Robots & robots,
  ...
  static std::shared_ptr<T> load(const mc_rbdyn::Robots & robots,
                                 const std::string & file);
