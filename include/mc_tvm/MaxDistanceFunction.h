...
/** This class implements a max distance function for two given objects */
class MC_TVM_DLLAPI MaxDistanceFunction : public tvm::function::abstract::Function
{
public:
  SET_UPDATES(MaxDistanceFunction, Value, Velocity, Jacobian, NormalAcceleration)

  /** Constructor
   *
   * \param c1 First max distance object
   *
   * \param c2 Second max distance object
   *
  ...
  /** First convex involved in the max distance */
  ...
  /** Second convex involved in the max distance */
...
using MaxDistanceFunctionPtr = std::shared_ptr<MaxDistanceFunction>;
